import { MultiQuoteRequest, NormalizedQuoteResult, NormalizedPlan } from "../../types/quote.types";

interface ProvinciaQuoteResponse {
...
export function mapToProvinciaRequest(input: MultiQuoteRequest): Record<string, unknown> {
  return {
    contacto: {
...
export function mapProvinciaResponse(raw: ProvinciaQuoteResponse): NormalizedQuoteResult {
  const plans: NormalizedPlan[] = (raw.planes ?? []).map((plan) => {

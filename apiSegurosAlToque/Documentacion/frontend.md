...
## Requerimiento al backend (catálogo Provincia)

Para sacar los códigos hardcodeados en frontend, necesitamos que el backend exponga un catálogo oficial de Provincia.

### 1) Marcas Provincia

- Método: `GET /api/v1/catalog/provincia/brands`
- Objetivo: devolver marcas habilitadas para cotización.
...
### 2) Modelos Provincia por marca

- Método: `GET /api/v1/catalog/provincia/models?brandCode=TOY`
- Objetivo: devolver modelos/versiones válidos para esa marca.
...
### 3) (Opcional recomendado) Búsqueda por texto

- Método: `GET /api/v1/catalog/provincia/search?query=captur`
- Objetivo: facilitar autocompletado y devolver código ya normalizado.
...
```json
{
  "success": true,
  "items": [
    { "brandCode": "REN", "brandName": "RENAULT", "modelCode": "123456", "modelName": "CAPTUR 2.0 INTENSE" }
  ]
}
```

### Contrato de error para catálogo

El frontend necesita diferenciar error técnico de "sin resultados":

- `400`: parámetros inválidos.
- `404`: marca no encontrada.
- `200` con `items: []`: sin modelos para esa marca.
- `500`: error interno.
...
```json
{
  ...
  "message": "No existe catálogo para brandCode RENX"
}
```

### Criterio mínimo de integración front

- El endpoint de cotización debe seguir igual (`POST /api/v1/quotes/auto/multi`).
- El frontend va a dejar de mandar `brandCode/modelCode` fijos apenas estén estos endpoints.
- Si catálogo falla, el front puede mantener fallback temporal con códigos conocidos para no cortar la operación.

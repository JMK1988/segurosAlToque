...
export class MaCotizadorService {
    ...
    /** GET /api/Vehiculos/marcas → string[] */
    getMarcas(): Observable<VehicleBrandsResponse> {
        return this.http.get<VehicleBrandsResponse>(`${BASE_URL}/api/Vehiculos/marcas`, { headers: this.headers });
    }

    /** GET /api/Vehiculos/modelos?marca={marca} → string[] */
    getModelos(marca: string): Observable<VehicleModelsResponse> {
        return this.http.get<VehicleModelsResponse>(`${BASE_URL}/api/Vehiculos/modelos`, {
            params: { marca },
            headers: this.headers
        });
    }

    /** GET /api/Vehiculos/versiones?marca={marca}&modelo={modelo} → VehicleVersion[] */
    getVersiones(marca: string, modelo: string): Observable<VehicleVersionsResponse> {
        return this.http.get<VehicleVersionsResponse>(`${BASE_URL}/api/Vehiculos/versiones`, {
            params: { marca, modelo },
            headers: this.headers
        });
    }

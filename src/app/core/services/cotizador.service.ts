...
export class CotizadorService {
    ...
    private readonly _datosWidget = signal<{ marca: string; modelo: string; version: string; anio: number } | null>(null);
    ...
    guardarDatosWidget(datos: { marca: string; modelo: string; version: string; anio: number } | null): void {
        ...
        const ma$ = this.maService.cotizar(maRequest).pipe(
            map((resp: any) => {
                ...
                if (resp.estado && resp.estado !== 'OK') {
                    const errorMsg = resp.mensaje || (resp.errores && resp.errores[0]) || 'Error en la consulta a la aseguradora.';
    ...
    private _getBeneficios(prod: string): string[] {
        ...
        const base = ['Responsabilidad Civil (Límite Legal)', 'Asistencia Jurídica', 'Asistencia Mecánica (200km)'];
        ...
        if (p === 'M BASICA' || p === 'M PLUS') {
            const mBase = [...base, 'Incendio Total/Parcial', 'Robo Total/Parcial', 'Destrucción Total por Accidente', 'Asistencia Mecánica (500km)'];
            if (p === 'M PLUS') {
                return [...mBase, 'Granizo (Sin límite)', 'Cristales Laterales y Traseros', 'Parabrisas', 'Cerraduras y Llaves', 'Daños por Inundación'];
            }
            return mBase;
        }
    ...
    private _normalizarMa(resp: MercantilAndinaResponse): Partial<ResultadoAseguradora> {
        const coberturas: CoberturaResultado[] = (resp.resultado || [])
            .filter(p => !p.error)
            .map(
                (p: MaProductoResultado): CoberturaResultado => {
                    const prod = p.producto?.toUpperCase() || '';
                    ...
                    const inspeccionApi = p.inspeccion?.opciones?.some((o) => o.id !== null && o.id !== '0') || false;
                    const requiereInspeccion = (prod !== 'A') &&
                        (inspeccionApi || p.inspeccion?.opciones?.length > 0);
